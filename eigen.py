...
def calculate_eigens(asset_pool_pd,tolerance):
    """
    input 1 : asset_pool_pd, n rows*m days, denoting prices of n assets on m days
...
def reduce_dimension(asset_pool_pd,vlist,k):
    """
    Select the top `k` eigenvector to reduce dimensions.  
    Return a dimension-reduced matrix from the original price matrix asset_pool_pd
    """
    sub_vector=np.array(vlist[0:k]).transpose()
    pricedata=asset_pool_pd.values.transpose()
    lower_dim_mat=np.dot(pricedata,sub_vector)
    return lower_dim_mat